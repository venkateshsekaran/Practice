...
const DriverList = () => {
    ...
    const [editObj, editObjUpdate] = useState({ id: '', make: '', size: '', wattage: '' })
    ...
    useEffect(() => {
        ...
        const url = `${config.BASE_API_URL}common/getDriverList`
        const data = {}
    ...
    const handleEdit = (item) => {
        ...
        const url = `${config.BASE_API_URL}common/updateDriverList`
        const data = { driverData: editObj }
        const configs = { headers: { authorization: localStorage.getItem('token') } }
    ...
    const handleDelete = (item) => {
        ...
        const url = `${config.BASE_API_URL}common/deleteDriver`
        const data = { driverId: item }
        const configs = { headers: { authorization: localStorage.getItem('token') } }
        Axios.post(url, data, configs)
            .then((res) => {
    ...
    return (
        <AdminLayout>
            ...
            <div className="float-right">
                ...
                <Button onClick={() => { history.push('/driver-table') }}
                    variant="primary" >
                    Add Driver Details </Button>
            </div>
